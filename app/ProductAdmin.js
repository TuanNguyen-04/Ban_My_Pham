...
export default function ProductAdmin() {
    ...
    const fetchProducts = async () => {
        try {
            const res = await fetch('http://103.249.117.201:12732/products');
    ...
    const handleDelete = async (id) => {
        Alert.alert('Xác nhận', 'Bạn có chắc muốn xóa sản phẩm này?', [
            { text: 'Hủy', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        await fetch(`http://103.249.117.201:12732 /products/${id}`, { method: 'DELETE' });
                        fetchProducts();
                    } catch {
    ...
    const handleSave = async () => {
        try {
            await fetch(`http://103.249.117.201:12732 /products/${editingProduct._id}`, {
                method: 'PUT',
